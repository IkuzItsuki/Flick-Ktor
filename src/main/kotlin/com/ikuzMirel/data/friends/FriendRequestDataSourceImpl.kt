...
class FriendRequestDataSourceImpl(
    ...
    override suspend fun sendFriendRequest(friend: FriendRequest): Boolean {
        val findSendFR = friendRequests.findOne(FriendRequest::senderId eq friend.senderId, FriendRequest::receiverId eq friend.receiverId)
        val findReceiveFR = friendRequests.findOne(FriendRequest::senderId eq friend.receiverId, FriendRequest::receiverId eq friend.senderId)
    ...
    override suspend fun acceptFriendRequest(requestId: String): Boolean {
        val requestObjectId = ObjectId(requestId)
        return friendRequests.updateOne(FriendRequest::id eq requestObjectId, setValue(FriendRequest::status, "Accepted" )).wasAcknowledged()
    }

    override suspend fun declineFriendRequest(requestId: String): Boolean {
        val requestObjectId = ObjectId(requestId)
        return  friendRequests.updateOne(FriendRequest::id eq requestObjectId, setValue(FriendRequest::status, "Declined")).wasAcknowledged()
    }
}